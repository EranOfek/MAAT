...
%            'cellremove3dots' - Read each line into a cell element, and
%                     combine lines that ends with "..." with the next line.
   ...
   switch lower(ReadType)
    ...
    case 'cellremove3dots'
       while (feof(FID(If))==0)
          ...
          while ~isempty(I3dot)
              % 3 dots found combine with next line
              LineNext = fgetl(FID(If));      
              Line     = sprintf('%s %s',Line(1:I3dot-1),LineNext);
              I3dot = strfind(Line,'...');
          end
