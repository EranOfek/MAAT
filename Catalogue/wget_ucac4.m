function [Cat,ColCell,Col,UnitsCell,Comments]=wget_ucac4(RA,Dec,SearchSize,SearchShape,OutType)
%------------------------------------------------------------------------------
...
%          - Output type: 'astcat'|'mat'. Default is 'mat'.
...
sprintf('%s -c %s %s %s %7.1f -m %d',Prog, RA, Dec, ShapeKey, SearchSizeAS, MaxNumStar)
%Res
...
switch lower(OutType)
    case 'astcat'
        % convert to an AstCat object
        AstC          = AstCat;
        AstC.Cat      = Cat;
        AstC.ColCell  = ColCell;
        AstC.Col      = Col;
        AstC.ColUnits = UnitsCell;
        Cat           = AstC;
