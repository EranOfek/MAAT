function [IsKeyVal,IsEqual]=iskeyval_substr(Head,varargin)
%--------------------------------------------------------------------------
% iskeyval_substr function                                     class/@HEAD
% Description: Check if header keywords in an HEAD object are equal to
%              some values, or contain a specific substring.
...
% Example: IsKeyVal=iskeyval_substr(Head,'SLITGRAB','dep','DICHTRAN','dep')
%          IsKeyVal=iskeyval_substr(Head,'EXPTIME',60,'FILTER','R')
% Reliable: 2
%--------------------------------------------------------------------------
...
for Ih=1:1:Nh,
    ...
    for Ikey=1:1:Nkey,
        ...
        if (ischar(HeadKeyVal) && ischar(RequestedVal)),
            % value is string
            IsEqual(Ih,Ikey) = ~isempty(strfind(Util.string.spacedel(HeadKeyVal),RequestedVal));
